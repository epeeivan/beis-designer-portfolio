...
<template>
    <div class="space-y-5">
        ...
        <div class="flex flex-wrap space-x-10">
            <img v-for="(software, index) in softwares.ui_ux" :key="`ui-ux-sofware-${index}`" class=" block h-[70px]  my-auto" 
                :src="`/public/images/${software?.img}.png`" alt="">
        </div>

        <h1 class="text-lg capitalize font-semibold">{{ $t('labels.acheivments') }}</h1>

        <div class="grid grid-cols-3 gap-10">
            <CardImage v-for="(acheivment,index) in acheivments.ui_ux" :key="`ui-ux-acheivments-${index}`" :item='acheivment'/>
        </div>
    </div>
</template>